...
class IFPipelineTesterConfig(BasePipelineTesterConfig):
    pipeline_class = IFPipeline
    required_input_params_in_call_signature = frozenset(
        ["prompt", "guidance_scale", "negative_prompt", "prompt_embeds", "negative_prompt_embeds"]
    )
...
@skip_mps
class TestIFPipeline(IFPipelineTesterConfig, PipelineTesterMixin):
    def test_inference(self):
        ...
        # fmt: off
        expected_slice = torch.tensor([-1.0000, -1.0000, -0.2177, -0.8947, 0.8860, 1.0000, -0.5022, -1.0000, 0.9550, -0.9627, -1.0000, 0.9768, -1.0000, -0.9924, -1.0000, -1.0000])
        # fmt: on
...
@skip_mps
class TestIFPipelineMemory(IFPipelineTesterConfig, PipelineOffloadTesterMixin):
    pass


@slow
@require_torch_accelerator
class TestIFPipelineSlow:
